...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!sg_csrf_check($_POST['csrf'] ?? '')) {
        ...
        $error = 'Şifrələr üst-üstə düşmür.';
    } else {
        $result = sg_customer_register($_POST['name'] ?? '', $_POST['phone'] ?? '', $_POST['email'] ?? '', $_POST['password'] ?? '');
        if ($result['ok']) {
            header('Location: account.php');
            exit;
        }
...
<title>Qeydiyyat — <?php echo h($restaurantName); ?></title>
...
<div class="auth-wrap">
  ...
  <div class="auth-card">
    ...
    <form method="post">
      ...
      <div class="field-block">
        <label>Telefon</label>
        <input type="tel" name="phone" placeholder="050 123 45 67" value="<?php echo h($_POST['phone'] ?? ''); ?>" required>
      </div>
      <div class="field-block">
      ...
      <button type="submit" class="btn btn-primary" style="width:100%; justify-content:center;">Qeydiyyatdan keç</button>
    </form>
    <div class="auth-foot">Artıq hesabınız var? <a href="login.php">Daxil olun</a></div>
  </div>
</div>
</body>
</html>
